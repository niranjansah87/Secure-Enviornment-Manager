...
SEM supports **three login modes** selectable from the login page:

| Mode | Icon | Description | Best For |
|------|------|-------------|----------|
| **Admin Access** | Shield | Dashboard password OR master API token from `.env` | Admin setup, full access |
| **API Key** | Key | API key generated by admin (can be bound to a user) | CI/CD, programmatic access |
| **User Login** | User | Username + password (PBKDF2 hashed, per-user salt) | Daily developer use |

### User Account Features
- **Role-Based Access**: `admin` and `developer` roles with scoped permissions
- **Namespace/Environment Selector**: Rich checkbox UI grouped by namespace when creating/editing users
- **Per-User Salt**: Each password hashed with a unique 16-byte random salt (480k PBKDF2-SHA256 iterations)
- **Forced Password Change**: Temp password on first login — full-screen overlay until changed
- **Change Password**: Available from sidebar user menu for voluntary password changes
- **Email Integration**: Optional SMTP for welcome/reset emails (non-blocking if not configured)

### API Key Features
- **User Binding**: API keys can be bound to a user — JWT carries user identity on key login
- **Inherited Scopes**: Bound keys auto-inherit user's namespace/environment access (extendable)
- **One-Time View**: Raw API key shown only at creation — Key ID is just an identifier, not a secret
- **Permanent Deletion**: Revoking a key permanently deletes it from storage
