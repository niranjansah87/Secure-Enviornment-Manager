...
const MODES: { id: LoginMode; label: string; icon: React.ReactNode; desc: string }[] = [
  { id: "admin",  label: "Admin Access", icon: <ShieldCheck className="w-3.5 h-3.5" />, desc: "Dashboard password or master API token" },
  { id: "apikey", label: "API Key",      icon: <KeyRound className="w-3.5 h-3.5" />,     desc: "API key generated by admin" },
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback to legacy API key path for admin mode only
      if (mode === "admin") {
        try {
