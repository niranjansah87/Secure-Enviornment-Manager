...
/// Mixin for blocs that need lifecycle-aware secrets hiding
mixin SecretsLifecycleMixin<T extends StatefulWidget> on State<T> {
  final LifecycleSecurityManager _lifecycleManager = LifecycleSecurityManager.instance;
  bool _wasBackgrounded = false;
  Set<String> _revealedSecretIdsSnapshot = {};

  @protected
  void Function(Set<String>)? onSecretsShouldHide;

  void initSecretsLifecycle(Set<String> initialRevealedIds) {
    _revealedSecretIdsSnapshot = Set.from(initialRevealedIds);
    WidgetsBinding.instance.addObserver(this);
    _lifecycleManager.addListener(_onSecretsLifecycleChanged);
  }

  void _onSecretsLifecycleChanged(SecureLifecycleState state) {
    if (state == SecureLifecycleState.backgrounded) {
      _wasBackgrounded = true;
      // Signal that secrets should be hidden
      onSecretsShouldHide?.call({});
    } else if (state == SecureLifecycleState.active && _wasBackgrounded) {
      _wasBackgrounded = false;
    }
  }

  void disposeSecretsLifecycle() {
    WidgetsBinding.instance.removeObserver(this);
    _lifecycleManager.removeListener(_onSecretsLifecycleChanged);
  }
